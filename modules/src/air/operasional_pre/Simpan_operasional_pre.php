...
if (empty($params['case']))
	...
	exit();
		...
		$KPE_AIR_FLOWMETER_OPERASIONAL_PRE_FLOW_RW_PENGKONDISIAN = $input['KPE_AIR_FLOWMETER_OPERASIONAL_PRE_FLOW_RW_PENGKONDISIAN'];
		...
		if($input['KPE_AIR_FLOWMETER_OPERASIONAL_PRE_ID']=="")
		{
			...
			if ($result_d >= 1) {
				...
				$sql_g = "SELECT KPE_AIR_FLOWMETER_OPERASIONAL_PRE_TANGGAL FROM KPE_AIR_FLOWMETER_OPERASIONAL_PRE WHERE RECORD_STATUS='A' ORDER BY KPE_AIR_FLOWMETER_OPERASIONAL_PRE_TANGGAL DESC";

				$this->MYSQL = new MYSQL();
				$this->MYSQL->database = $this->CONFIG->mysql_koneksi()->db_nama;
				$this->MYSQL->queri = $sql_g;
				$result_g = $this->MYSQL->data()[0]['KPE_AIR_FLOWMETER_OPERASIONAL_PRE_TANGGAL'];
				$datesebelumnya = Date('Y/m/d',strtotime($result_g.'+1 day'));
				if ($datesebelumnya != $input['KPE_AIR_FLOWMETER_OPERASIONAL_PRE_TANGGAL']) {
					$this->callback['respon']['pesan'] = "gagal";
					$this->callback['respon']['text_msg'] = "Tanggal $datesebelumnya Belum terisi";
				} else {
					$sql_h = "SELECT DISTINCT KPE_AIR_FLOWMETER_REKAP_USED_PRE_TANGGAL FROM KPE_AIR_FLOWMETER_REKAP_USED_PRE WHERE KPE_AIR_FLOWMETER_REKAP_USED_PRE_TANGGAL='".$input['KPE_AIR_FLOWMETER_OPERASIONAL_PRE_TANGGAL']."' AND RECORD_STATUS='A' ORDER BY KPE_AIR_FLOWMETER_REKAP_USED_PRE_TANGGAL DESC";

					$this->MYSQL = new MYSQL();
					$this->MYSQL->database = $this->CONFIG->mysql_koneksi()->db_nama;
					$this->MYSQL->queri = $sql_h;
					$result_h = $this->MYSQL->data()[0]['KPE_AIR_FLOWMETER_REKAP_USED_PRE_TANGGAL'];
					$datesebelumnya = Date('Y/m/d',strtotime($result_g.'+1 day'));
					if ($result_h == "") {
						$this->callback['respon']['pesan'] = "gagal";
						$this->callback['respon']['text_msg'] = "Catatan tanggal $datesebelumnya belum direkap";
						$this->callback['result'] = $sql_h;
					} else {
						$data_master = array(
							...
							'KPE_AIR_FLOWMETER_OPERASIONAL_PRE_FLOW_RW_PENGKONDISIAN' => $KPE_AIR_FLOWMETER_OPERASIONAL_PRE_FLOW_RW_PENGKONDISIAN,
