...
$sql_a = "SELECT * FROM KPE_KWH_RUMUS WHERE KPE_KWH_FLOWMETER_ID='20210331163116926314' AND 
...
for ($i = 0; $i < $len; $i++) {
    ...
    if($data[$i]['KPE_KWH_RUMUS_FORMULA'] == "+"){
    	$current_running_sum += $value;
    } else if ($data[$i]['KPE_KWH_RUMUS_FORMULA'] == "-") {
    	$current_running_sum -= $value;
    } else if ($data[$i]['KPE_KWH_RUMUS_FORMULA'] == "*") {
    	$current_running_sum *= $value;
    }
