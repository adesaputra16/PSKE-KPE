<?php
  $input_option=array();
  $params=array(
    //'case'=>"ROsensi_lembur_spl_pdf_nonlogin",
  ...
  // echo "<RO>".print_r($respon_ctt['result'],true)."</RO>";
//exit();
?>
...
<script>
  ...
  function listRekap() {
    ...
    $.ajax({
      ...
      url:refseeAPI,
      dataType:'json',
      data:'aplikasi=<?php echo $d0;?>&ref=list_rekap_used_ro&dateRangeS='+dateRangeS+'&dateRangeSE='+dateRangeE+'&MULTI_FILTER='+multiFilter+'&BULAN_FILTER='+$('#BULAN_FILTER').val()+'&TAHUN_FILTER='+$('#TAHUN_FILTER').val(),
        ...
        if(data.respon.pesan=="sukses")
        {
          ...
          for (let i = 0; i < data.result.length; i++) {
            if (data.result[i].REKAP == "") {
              ...
              codding = data.result[i].REKAP[0].KPE_AIR_FLOWMETER_REKAP_USED_RO_CODDING;
              flowBeban = '';
              flowTotal = data.result[i].REKAP[0].KPE_AIR_FLOWMETER_REKAP_USED_RO_TOTAL;
              flowRataRata = data.result[i].REKAP[0].KPE_AIR_FLOWMETER_REKAP_USED_RO_RATA_RATA;
              listKosong = '';
            }
            let rowspan = 1 + data.result[i].REKAP.length;
            listData += /*html*/`<tr rowspan="${rowspan}">
                                  <td rowspan="${rowspan}" class="fixed-top bordered text-center ${listKosong}"><button class="btn btn-danger hapus" KPE_AIR_FLOWMETER_REKAP_USED_RO_TANGGAL="${data.result[i].TANGGAL_DEL}" KPE_AIR_FLOWMETER_REKAP_USED_RO_CODDING="${codding}"><i class="fa fa-trash"></i> Hapus</button></td>
            ...
            for (let j = 0; j < data.result[i].REKAP.length; j++) {
              listData += /*html*/`<tr>
                                      ...
                                      <td class="bordered">${data.result[i].REKAP[j].KPE_AIR_FLOWMETER_REKAP_USED_RO_BEBAN}</td>
                                    </tr>`;
      ...
      error:function(x,e){
        error_handler_json(x,e,'=> list_rekap_used_RO()');
      }//end error
    });
  }
  //?=============== END LIST REKAP ==================?//
  ...
  $('tbody').on('click', 'button.hapus', function(){
    let KPE_AIR_FLOWMETER_REKAP_USED_RO_TANGGAL = $(this).attr('KPE_AIR_FLOWMETER_REKAP_USED_RO_TANGGAL');
    let KPE_AIR_FLOWMETER_REKAP_USED_RO_CODDING = $(this).attr('KPE_AIR_FLOWMETER_REKAP_USED_RO_CODDING');
    // console.log(KPE_AIR_FLOWMETER_REKAP_USED_RO_CODDING);
    Swal.fire({
      ...
      if (result.isConfirmed) {
        $.ajax({
          ...
          url:refseeAPI,
          dataType:'json',
          data:'aplikasi=<?php echo $d0;?>&ref=hapus_rekap_used_ro&KPE_AIR_FLOWMETER_REKAP_USED_RO_TANGGAL='+KPE_AIR_FLOWMETER_REKAP_USED_RO_TANGGAL+'&KPE_AIR_FLOWMETER_REKAP_USED_RO_CODDING='+KPE_AIR_FLOWMETER_REKAP_USED_RO_CODDING,
          success:function(data)
          { 
            if(data.respon.pesan=="sukses")
            {
